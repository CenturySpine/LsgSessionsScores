...
fun PlayerDetailScreen(
    ...
    val userState by playerViewModel.getPlayerById(userId).collectAsState(initial = null)

    // Early return pour l'état loading/null
    when (val user = userState) {
        null -> {
            ...
            // L'utilisateur existe, on peut continuer avec user de type Player (non-null)
            PlayerDetailContent(
                user = user, // ✅ user est non-null ici
                navController = navController,
                playerViewModel = playerViewModel
            )
        }
...
private fun PlayerDetailContent(
    ...
    val currentIndex = sortedUsers.indexOfFirst { it.id == user.id }
