...
class AppUserDaoSupabase @Inject constructor(
    ...
    suspend fun ensureUserRow(): AppUser? {
        ...
        return try {
            ...
            if (existing == null) {
                ...
                // Note: Player creation is now deferred until city selection is completed by the user
            } else {
    ...
    suspend fun hasLinkedPlayer(): Boolean {
        val userId = try {
        ...
        return try {
            ...
            Log.w("AppUserDao", "hasLinkedPlayer failed: ${t.message}")
            false
        }
    }
    ...
    suspend fun createPlayerForCurrentUser(cityId: Long): Boolean {
        val session = supabase.auth.currentSessionOrNull()
        val user = session?.user ?: return false
        val id = user.id
        val email = user.email

        // Defensive extraction of metadata
        fun anyToStringCompat(value: Any?): String? {
            if (value == null) return null
            return when (value) {
                is String -> value
        ...
        return try {
            // 1) Create a new player with the provided cityId
            try {
                ...
                Log.w(
                    "AppUserDao",
                    "createPlayerForCurrentUser: insert failed (user_id=$id, name=$defaultName, cityId=$cityId): ${t.message}",
                    t
                )
                return false
            }

            // 2) Retrieve the created player's id
            val createdPlayer = try {
                supabase.postgrest["players"].select {
                    filter { eq("user_id", id); eq("name", defaultName); eq("cityid", cityId) }
                ...
                Log.w(
                    "AppUserDao",
                    "createPlayerForCurrentUser: select failed (user_id=$id, name=$defaultName, cityId=$cityId): ${t.message}"
                )
                null
            }
            ...
            if (playerId != null && playerId > 0) {
                // 3) Create the link in user_player_link
                try {
                    ...
                    Log.w(
                        "AppUserDao",
                        "createPlayerForCurrentUser: link insert failed (user_id=$id, player_id=$playerId): ${t.message}",
                        t
                    )
                    ...
                    try {
                        ...
                        Log.w(
                            "AppUserDao",
                            "createPlayerForCurrentUser: link update failed (user_id=$id, player_id=$playerId): ${t2.message}",
                            t2
                        )
                        return false
                    }
            ...
            Log.w("AppUserDao", "createPlayerForCurrentUser failed: ${t.message}")
            false
        }
    }
